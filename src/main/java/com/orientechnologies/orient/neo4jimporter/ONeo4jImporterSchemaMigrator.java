...
class ONeo4jImporterSchemaMigrator {
  ...
  public ONeo4jImporterSchemaMigrator(
      String keepLogString,
      DecimalFormat df,
      ODatabaseDocument oDb,
      ONeo4jImporterStatistics statistics) {
  ...
  public void invoke(Session neo4jSession) throws Exception {
    ...
    try {
      ...
      String logString =
          PROGRAM_NAME + " - v." + OConstants.getVersion() + " - PHASE 3 completed!\n";
  ...
  private void importIndices(Session session) {
    ...
    try {
      ...
      indices =
          session
              .run(query)
              .list(); // storing result in a list in order to avoid another similar query later.
      // List's dimension is not a problem.
    ...
    try {
      ...
      for (Record currentIndexDefinition : indices) {
        ...
        String neo4jLabelOfIndex =
            indexDescription.substring(
                indexDescription.indexOf(":") + 1, indexDescription.indexOf("("));
        orientDBIndexClassName = neo4jLabelOfIndex;

        ONeo4jImporterContext.getInstance()
            .getMessageHandler()
            .debug(this, "all index: on label " + neo4jLabelOfIndex);

        statistics.neo4jNonConstraintsIndicesCounter++;

        ONeo4jImporterContext.getInstance()
            .getMessageHandler()
            .debug(this, "non constraint index: on label " + neo4jLabelOfIndex);
        ...
        // create the index in OrientDB - we create NOT UNIQUE indices here (case of UNIQUE indices
        // is handled above)

        // create class neo4jLabelOfIndex
        // there might be in fact cases where in neo4j the index has been defined, but no nodes have
        // been created.
        // As a result, no nodes of that class have been imported in OrientDB, so that class does
        // not exist in Orient yet
        if (!oDb.getMetadata().getSchema().existsClass(neo4jLabelOfIndex)) {
          orientDBIndexClassName =
              ONeo4jImporterUtils.checkVertexClassCreation(neo4jLabelOfIndex, oDb);
        }

        // creates in OrientDB all the properties this index is defined on
        for (int i = 0; i < properties.length; i++) {
          propertyCreationSuccess =
              ONeo4jImporterUtils.createOrientDBProperty(
                  session,
                  neo4jLabelOfIndex,
                  orientDBIndexClassName,
                  properties[i],
        ...
        try {
          ...
          OClass orientDBClass = oDb.getMetadata().getSchema().getClass(orientDBIndexClassName);

          // if the index is created as a side effect of the creation of a uniqueness constraint, we
          // handled the case already above
          ...
          if (!orientDBClass.areIndexed(propertiesList)) {
            ...
            oDb.command(statement);

            statistics.orientDBImportedIndicesCounter++;
          }
        } catch (Exception e) {
          String mess =
              "Found an error when trying to create a NOTUNIQUE Index in OrientDB. Node label '"
                  + neo4jLabelOfIndex
                  + "': "
                  + e.getMessage();
          ONeo4jImporterContext.getInstance().printExceptionMessage(e, mess, "error");
  ...
  private String importConstraints(Session session) throws Exception {
    ...
    if (indexWorkaround) {
      if (oDb.getMetadata().getSchema().existsClass("MultipleLabelNeo4jConversion")) {
        OClass multipleLabelClass =
            oDb.getMetadata().getSchema().getClass("MultipleLabelNeo4jConversion");

        if (multipleLabelClass.existsProperty("neo4jLabelList")) {
          if (oDb.getMetadata()
              .getIndexManager()
              .existsIndex("MultipleLabelNeo4jConversion.neo4jLabelList")) {

            logString =
                "Rebuilding Index MultipleLabelNeo4jConversion.neo4jLabelList. Please wait...\n";
            ONeo4jImporterContext.getInstance().getMessageHandler().info(this, logString);
            ...
            try {
              oDb.getMetadata()
                  .getIndexManager()
                  .getClassIndex(
                      "MultipleLabelNeo4jConversion", "MultipleLabelNeo4jConversion.neo4jLabelList")
                  .rebuild();
              ONeo4jImporterContext.getInstance()
                  .getMessageHandler()
                  .info(this, "\r" + logString + "Done\n");
            } catch (Exception e) {
              ONeo4jImporterContext.getInstance()
                  .getMessageHandler()
                  .info(this, "\r" + logString + "Failed\n");
              String mess =
                  "Found an error when trying to rebuild the index 'MultipleLabelNeo4jConversion.neo4jLabelList': "
                      + e.getMessage();
              ONeo4jImporterContext.getInstance().printExceptionMessage(e, mess, "error");
              ONeo4jImporterContext.getInstance().printExceptionStackTrace(e, "error");
            }
    ...
    try {
      ...
      while (result.hasNext()) {
        ...
        // determine the type of the constraints - different actions will need to be taken according
        // to this type
        ...
        // Neo4j allows constraints on both nodes and relationship. To get the OrientDB class, we
        // have to separate the cases
        String orientDBIndexClass = "";
        String neo4jLabel = "";

        try {
          // we can get the label with the method getLabel() only if this constraint is associated
          // with a node
          ...
          // there might be in fact cases where in neo4j the constraint as been defined, but no
        ...
        try {
          // we can get the relationship this constraint is associated with only if it is associated
          // with a relationship

          String neo4jConstraintRelationshipType =
              this.getRelationshipType(neo4jConstraintDefinition);
          ...
          if (neo4jConstraintRelationshipType != null) {
            ...
            // there might be in fact cases where in neo4j the constraint as been defined, but no
            // nodes have been created.
            // As a result, no nodes of that class have been imported in OrientDB, so that class
            // does not exist in Orient
        ...
        // we now know the type of this constraints and the class on which it is defined
        // (orientDBIndexClass)
        ...
        propertyCreationSuccess =
            ONeo4jImporterUtils.createOrientDBProperty(
                session, neo4jLabel, orientDBIndexClass, neo4jPropKey, oDb, neo4jConstraintType);

        // now that the property has been created, we need to take actions based on the
        // neo4jConstraintType
        if (propertyCreationSuccess) {
          ...
          if ("UNIQUENESS".equals(neo4jConstraintType)) {
            ...
            try {
              ...
              // unique constraints can be only on nodes. with this check we avoid odd things in
              // very odd situations that may happen
              if (isConstraintsOnNode) {

                // we check that orientDBIndexClass is a vertex class; with this check we avoid odd
                // things in very odd situations that may happen,
                // e.g. node labels = rel types.  nodes with multiple types (that are imported in a
                // single class).
                // In such case it may happen that it try to create an index on the class, but that
                // class does not exist as vertex class
                // (because all nodes were imported into another class - but it exists as edge
                // class)
                if (oDb.getMetadata()
                    .getSchema()
                    .getClass(orientDBIndexClass)
                  ...
                  if (!oDb.getMetadata()
                      .getSchema()
                      .getClass(orientDBIndexClass)
                      .areIndexed(neo4jPropKey)) {
                    orientDBIndex =
                        oDb.getMetadata()
                            .getSchema()
                            .getClass(orientDBIndexClass)
                            .getProperty(neo4jPropKey)
                            .createIndex(
              ...
              String mess =
                  "Found an error when trying to create a UNIQUE Index in OrientDB. Corresponding Property in Neo4j is '"
                      + neo4jPropKey
                      + "' on node label '"
                      + orientDBIndexClass
                      + "': "
        ...
        if ("UNIQUENESS".equals(neo4jConstraintType)) {
          //          keepLogString =
          //              df.format(statistics.orientDBImportedUniqueConstraintsCounter) + "
          // OrientDB UNIQUE Indices have been created";
        }
        //        if ("NODE_PROPERTY_EXISTENCE".equals(neo4jConstraintType)) {
        //        }
        //        if ("RELATIONSHIP_PROPERTY_EXISTENCE ".equals(neo4jConstraintType)) {
  ...
  private String getConstraintLabel(Map<String, Object> neo4jConstraintDefinition) {
    ...
    String constraintDescription = neo4jConstraintDefinition.get("description").toString();
    String label =
        constraintDescription.substring(
            constraintDescription.indexOf("(") + 1, constraintDescription.indexOf(")"));
    label = label.replace(" ", "");
  ...
  private String getRelationshipType(Map<String, Object> neo4jConstraintDefinition) {
    ...
    if (constraintDescription.contains("exists(")) {
      relationshipType =
          constraintDescription.substring(
              constraintDescription.indexOf("exists(") + 1, constraintDescription.lastIndexOf(")"));
  ...
  private String getConstraintProperty(Map<String, Object> neo4jConstraintDefinition) {
    ...
    String propertyName =
        constraintDescription.substring(
            constraintDescription.indexOf(") ASSERT ") + 1, constraintDescription.indexOf("IS"));
