...
public class ONeo4jImporterUtils {
   ...
   * empty set of nodes. Thus is used when no OrientDB class was defined for an empty set of nodes,
   * but it must be defined due to some constraints or indexes.
   *
   * <p>It checks if the nodes with label passed as parameter have been imported in the
   * MultipleLabelNeo4jConversion class. If no vertices were imported with the passed label (among
   * the other labels) the correspondent class does not exist, and so: 1. the vertex class is
   * created as part of the schema migration 2. its indices (and corresponding properties) are
   * created
   */
  public static String checkVertexClassCreation(String neo4jLabel, ODatabaseDocument odb) {
    ...
    String sqlQuery =
        "SELECT @rid FROM V WHERE @class = 'MultipleLabelNeo4jConversion' AND neo4jLabelList CONTAINS '"
            + neo4jLabel
            + "' LIMIT 1";
    ...
    while (vertices.hasNext()) {
      ...
      ONeo4jImporterContext.getInstance()
          .getMessageHandler()
          .debug(ONeo4jImporterUtils.class, "\n\n\nfound it: " + orientDBClassName + " " + v);
      orientDBClassName = "MultipleLabelNeo4jConversion";
    }

    if (u == 0) {

      // case: the label is mapped to a single OrientDB class, and it will be created as not present
      // in the target database yet.
      // In fact this method is called when the class does not exist in orientDB, because no nodes
      // in neo4j are logically connected to it, but just an index some constraints.
      ...
      // in order to improve record lookup when filtering on Neo4j labels all classes must have the
      // neo4jLabelList properties and an index on it
      // when classes are created during nodes migration, this property is created automatically
      // here we need to create it for those additional classes that are created (empty) during
      // schema migration
      try {
  ...
  public static boolean createOrientDBProperty(
      Session session,
      String neo4jLabel,
    ...
    // To do this we will use java instanceof, as there are no specific methods in the noe4j api to
      ...
      while (result.hasNext()) {
        ...
        try {
          ...
          ONeo4jImporterContext.getInstance()
              .getMessageHandler()
              .debug(
                  ONeo4jImporterUtils.class,
                  "debugCounter is: "
                      + debugCounter
                      + ". Working on node "
                      + currentNode.get("id").asString());
          ...
          if (propertyValue != null) {
            ...
            // not all nodes with label myNeo4jLabel may have this property - even if we have a
            // unique constraint on this property
            // (it is unique in the nodes where the property exists). When we find a node with this
            // property, we exit the loop
            foundNode = true;
          }
    ...
    // However, there may be cases where the constraints has been defined, but no nodes have been
    // created yet. In this case we cannot know the data type. We will use STRING as default
    if (foundNode == false) {
    ...
    try {
      ...
      // oDb.getRawGraph().getMetadata().getSchema().getClass(orientDBIndexClass)
      ...
      if (!foundNode) {
        ...
        ONeo4jImporterContext.getInstance()
            .getMessageHandler()
            .info(ONeo4jImporterUtils.class, logString);
      } else {
        logString =
            "Created Property '"
                + neo4jPropKey
        ...
        ONeo4jImporterContext.getInstance()
            .getMessageHandler()
            .debug(ONeo4jImporterUtils.class, logString);
      }
      return true;
    } catch (Exception e) {
      String mess =
          "Found an error when trying to create a Property in OrientDB. Corresponding Property in Neo4j is '"
              + neo4jPropKey
              + "' on node label '"
              + orientDBIndexClass
              + "': "
