...
public class OFunctionsHandler {
  ...
  public static String getHMSFormat(Date start, Date end) {
    ...
    String hmsTime = String.format("%02d:%02d:%02d", TimeUnit.MILLISECONDS.toHours(end.getTime() - start.getTime()),
        TimeUnit.MILLISECONDS.toMinutes(end.getTime() - start.getTime()) - TimeUnit.HOURS
            .toMinutes(TimeUnit.MILLISECONDS.toHours(end.getTime() - start.getTime())),
        TimeUnit.MILLISECONDS.toSeconds(end.getTime() - start.getTime()) - TimeUnit.MINUTES
            .toSeconds(TimeUnit.MILLISECONDS.toMinutes(end.getTime() - start.getTime())));
  ...
  public static String getHMSFormat(long millis) {
    ...
    String hmsTime = String.format("%02d:%02d:%02d", TimeUnit.MILLISECONDS.toHours(millis),
      ...
      if (!d2.toMap().keySet().contains(key1)) {
        ...
        if (obj1 instanceof String && obj2 instanceof String) {    // base case: value is a string
          if (!obj1.equals(obj2)) {
            ...
            && obj2 instanceof ODocument) {   // inductive case: two values are documents, recursive comparison
